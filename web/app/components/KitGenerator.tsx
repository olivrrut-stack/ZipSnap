    ...
    try {
      ...
      for (;;) {
        ...
        setJob({
          ...
          images: s.images ?? [], copy: s.copy, manifestHealth: s.manifestHealth, iconKit: s.iconKit, growthReport: s.growthReport,
        });
...
function Results({
  ...
  const [report, setReport] = useState<GrowthReportData | undefined>(job.growthReport);
  const [regenReport, setRegenReport] = useState(false);
  const [regenReportError, setRegenReportError] = useState<string | null>(null);
  ...
  async function regenerateReport() {
    setRegenReport(true);
    setRegenReportError(null);
    try {
      const res = await fetch(`${WORKER}/api/jobs/${job.id}/regenerate-report`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
      ...
      setRegenReportError("Regeneration failed. Please try again.");
    } finally {
    ...
    <div className="panel">
      ...
      {report && (
        <>
          ...
          {regenReportError && <div className="cb-error">{regenReportError}</div>}
